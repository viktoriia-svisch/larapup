...
class ConfigValidator
{
    ...
    public function validate($file, $arrayLoaderValidationFlags = ValidatingArrayLoader::CHECK_ALL)
    {
            ...
            foreach ($licenses as $license) {
                ...
                if ($spdxLicense && $spdxLicense[3]) {
                    if (preg_match('{^[AL]?GPL-[123](\.[01])?\+$}i', $license)) {
                        ...
                        $warnings[] = sprintf(
                            'License "%s" is a deprecated SPDX license identifier, see https:
                            $license
                        );
                    }
                }
        ...
        if (!empty($manifest['type']) && $manifest['type'] == 'composer-installer') {
            $warnings[] = "The package type 'composer-installer' is deprecated. Please distribute your custom installers as plugins from now on. See https:
        }
