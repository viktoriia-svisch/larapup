...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $config->prohibitUrlByConfig('http:
        ...
        $opts = stream_context_get_options(StreamContextFactory::getContext('http:
    ...
    private function checkHttp($proto, Config $config)
    {
        ...
        try {
            $this->rfs->getContents('packagist.org', $proto . ':
    ...
    private function checkHttpProxy()
    {
        ...
        try {
            $json = json_decode($this->rfs->getContents('packagist.org', $protocol . ':
            $hash = reset($json['provider-includes']);
            $hash = $hash['sha256'];
            $path = str_replace('%hash%', $hash, key($json['provider-includes']));
            $provider = $this->rfs->getContents('packagist.org', $protocol . ':
