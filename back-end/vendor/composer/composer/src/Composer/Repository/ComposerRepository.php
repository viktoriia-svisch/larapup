...
class ComposerRepository extends ArrayRepository implements ConfigurableRepositoryInterface
{
    ...
    protected function loadRootServerFile()
    {
        ...
        if ($this->allowSslDowngrade) {
            $this->url = str_replace('https:
            $this->baseUrl = str_replace('https:
        }
        ...
        if (preg_match('{^https?:
            $this->url = 'https:
            $this->baseUrl = 'https:
            $this->lazyProvidersUrl = $this->canonicalizeUrl('https:
    ...
    protected function fetchFile($filename, $cacheKey = null, $sha256 = null, $storeLastModifiedTime = false)
    {
        ...
        if (($pos = strpos($filename, '$')) && preg_match('{^https?:
        ...
        while ($retries--) {
            try {
                ...
                if ($sha256 && $sha256 !== hash('sha256', $json)) {
                    if ($this->allowSslDowngrade) {
                        $this->url = str_replace('http:
                        $this->baseUrl = str_replace('http:
                        $filename = str_replace('http:
                    }
                    if ($retries) {
                        usleep(100000);
                        continue;
